...
string_buffer*
stringBufferCreate(void* (*malloc_f)(size_t len))
{
  string_buffer* result = malloc_f(sizeof(string_buffer));
  if (result == NULL)
    {
      return NULL;
    }
...
char*
stringBufferToString(string_buffer* buffer, void* (*malloc_f)(size_t len), void (*free_f)(void* ptr))
{
  ...
  /*  char* string = realloc(result, buffer->string_length);
   ...
   free(buffer);
   return string;*/
  char* result = malloc_f(buffer->string_length * sizeof(char));
  if (result == NULL)
    {
      return NULL;
    }
...
void
stringBufferFree(string_buffer* buffer, void (*free_f)(void* ptr))
{
  if (buffer == NULL)
    {
