...
#ifndef JET_JSON_RAM
#define JET_JSON_RAM 256 * 2
#endif

#ifndef JET_MESSAGE_RAM
#define JET_MESSAGE_RAM 128 * 2
#endif

struct JetPeer;
typedef bool (*set_handler_t)(aJsonObject *val, void *context);
typedef void (*fetch_handler_t)(const char *path, const char *event,
                                aJsonObject *val, void *context);
...
struct JetPeer {
  ...
  JetState *state(const char *path, aJsonObject *val,
                  set_handler_t handler = NULL, void *context = NULL);
  JetFetcher *fetch(const char *path, fetch_handler_t handler,
                    void *context = NULL);
  JetFetcher *fetch(aJsonObject *expr, fetch_handler_t handler,
                    void *context = NULL);
